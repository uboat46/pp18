<template>
  <div class="login bg-itam">
    ...
    <main class="form">
      <div class="form__name color-itam w3-text-white">Registro</div>
      <div class="controls w3-white">
        ...
        <div class="controls__button">
          <input type="submit" name="submit" class="border-itam color-text-itam" value="Registrarse" :disabled="valid ? false:true" @click.prevent="register"/>
        </div>
        
        <div class="form__footer">
          <p>Ya tienes cuenta? <span class="color-text-itam"><router-link to="/">Inicia Sesión</router-link></span></p>
        </div>
      </div>
    </main>
  ...
  methods: {
    ...
    register () {
      ...
      firebase.auth().createUserWithEmailAndPassword(vm.email, vm.password).then(
        user => {
          this.$router.replace({name: 'FormaRegistro'})
        },
        err => {
